...
# Apply security fixes newer than the pinned base image before adding runtime packages.
RUN apk upgrade --no-cache && apk add --no-cache ca-certificates sqlite-libs tzdata
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
    CMD ["/bin/busybox", "wget", "-q", "-O", "/dev/null", "http://localhost:3000/healthz"]
